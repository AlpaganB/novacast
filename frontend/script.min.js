...
function isCacheValid(isoDate) {
  ...
  // Kademeli cache stratejisi
  if (daysDiff <= 3) return diffHours < 1; // Yakın tarih: 1 saat
  if (daysDiff <= 7) return diffHours < 3; // Orta vade: 3 saat
  return diffHours < 6; // Uzun vade: 6 saat
}
...
async function searchWeather(forceRefresh = false) {
  ...
  // Cache check
  if (!forceRefresh && currentCity && currentCity.toLowerCase() === city.toLowerCase() && currentForecastData && currentForecastData.gunluk) {
    if (isCacheValid(selectedISO)) {
      const cachedForecast = currentForecastData.gunluk.find(f => {
